...
# Background
In this project, we are expanding our current work projecting the distribution and abundance of marine fish and invertebrates throughout the U.S. Northeast Large Marine Ecosystem to include the southern Bay of Fundy and Nova Scotian waters Canada under potential future climate conditions. Working with folks from the Eastern Charlotte Waterways Program, we have identified a suite of species of particular interest within this region of the Gulf of Maine, including American lobster, Atlantic herring, Atlantic scallops. For each of these species, we will fit delta lognormal two-stage generalized additive models. This modeling approach has been widely used in other marine fish distribution modeling studies and has several advantages. First, the two stage approach models presence/absence and then models the log positive biomass observations, and this structure accommodates situations where the number of absence observations exceeds those expected from traditional “count” distributions (e.g., Poisson, tweedie). Second, the additive modeling framework requires no a priori assumptions about the functional relationships between the response (species presence/absence and biomass) and predictor variables, allowing for non-linear relationships. We will fit models to training data from 1982-2013 and then validate fitted models using testing data from 2013-2018. Finally, we will use the fitted models to project mid-century (2055) species distribution and abundance using sea surface temperatures from the RCP 8.5 "business as usual" climate model ensemble.
...
```{r}
# Load in the data
load(paste(lab.data.path, "Survdat_Nye_allseason_2019.RData", sep = ""))
...
dat<- survdat %>% 
  filter(., EST_YEAR >= 1982 & SEASON %in% c("FALL", "SPRING"))

# Clean up
rm(survdat)

# Filtering, don't really need ALL the species. Start with JHare assessment species...
spp.assmnt<- read_csv(paste(proj.path, "Data/Assesmentfishspecies.csv", sep = ""))

dat.sub<- dat %>%
  filter(., COMNAME %in% spp.assmnt$COMNAME)
...
dat.grp<- dat.sub %>%
  ...
  group_by(UNIQUE_ID, EST_YEAR, EST_MONTH, EST_DAY, EST_DATE, SEASON, DECDEG_BEGLAT, DECDEG_BEGLON, COMNAME) %>%
  summarize(., "BIOMASS.MOD" = sum(BIOMASS, na.rm = TRUE)) 
dat.grp$COMNAME<- as.character(dat.grp$COMNAME)
dat.grp$SEASON<- as.character(dat.grp$SEASON)

# Need to add null observations
null.dat<- expand.grid("UNIQUE_ID" = as.character(unique(dat.grp$UNIQUE_ID)), "COMNAME" = as.character(unique(dat.grp$COMNAME)))
...
dat.grp.join<- dat.grp %>%
  ...
  dplyr::select(., -c(COMNAME, SVSPP, BIOMASS.MOD))
...
# Join em 
null.dat.join$MatchCol<- paste(null.dat.join$UNIQUE_ID, null.dat.join$SEASON, null.dat.join$COMNAME, sep = "_")

dat.grp$MatchCol<- paste(dat.grp$UNIQUE_ID, dat.grp$SEASON, dat.grp$COMNAME, sep = "_")
...
if(plot.tf){
  ...
  for(i in 61:length(unique(dat.grp$COMNAME))){
    ...
    dat.use<- dat.grp %>%
      ungroup() %>%
      dplyr::filter(., COMNAME == unique(dat.grp$COMNAME)[i] & SEASON == unique(dat.grp$SEASON)[j])
    ...
    base<- ggplot() +
      ...
      geom_point(data = plot.zeros, aes(x = DECDEG_BEGLON, y = DECDEG_BEGLAT), shape = 3, size = 0.2) +
      coord_sf(xlim = xlim.use, ylim = ylim.use, expand = FALSE) +
       scale_x_continuous(breaks = c(-74, -68)) +
        scale_y_continuous(breaks = c(36, 40, 44)) +
      theme(axis.title.x = element_blank(), axis.title.y = element_blank(), panel.background = element_rect(fill = "white"), panel.border = element_rect(fill = NA), strip.background = element_rect(fill = "white")) +
      facet_wrap(~EST_YEAR, nrow = 4, ncol = 10)

    catch.plot<- base +
      geom_point(data = plot.pos, aes(x = DECDEG_BEGLON, y = DECDEG_BEGLAT, fill = BIOMASS.MOD.Size, size = BIOMASS.MOD.Size), alpha = 0.75, pch = 21) +
             scale_fill_gradient(name = "Catch_KG", low = "#deebf7", high = "#084594", limits = c(0.25, 0.9), breaks = c(0.25, 0.375, 0.5, 0.625, 0.9), labels = c("<=1", "<=5", "<=10", "<=100", ">500")) +
        scale_size_continuous(name = "Catch_KG", limits = c(0.25, 0.9), breaks = c(0.25, 0.375, 0.5, 0.625, 0.9), labels = c("<=1", "<=5", "<=10", "<=100", ">500")) +
       scale_x_continuous(breaks = c(-74, -68)) +
        scale_y_continuous(breaks = c(36, 40, 44)) +
      facet_wrap(~ EST_YEAR, nrow = 4, ncol = 10)

    ggsave(paste(proj.path, "Results/", unique(dat.use$COMNAME), unique(dat.use$SEASON), "CatchFullExt.jpg", sep = ""), width = 18, height = 10, catch.plot, dpi = 400)
    
    print(paste(unique(dat.use$COMNAME), unique(dat.use$SEASON), "is done!", sep = ""))
    }
  }
}
```
...
source(paste(lab.func.path, "ObservedSST_Function.R", sep = ""))
oisst<- env_data_extract(data.set = "OISST", dates = NULL, box = c(-77, -62, 34, 46.5), out.dir = paste(proj.path, "/Data/", sep = ""), mask = NULL)
```
...
cd.to.path<- list("RCP85" = "Box/RES Data/CMIP5_SST/RCP85/", "RCP45" = "Box/RES Data/CMIP5_SST/RCP45/", "Historical" = "Box/RES Data/CMIP5_SST/Historical/")
...
for(i in seq_along(cd.to.path)){
  ...
  system(paste("cd", paste("~/", "'", cd.to.path.use, "'", sep = ""), "&& cat > Std1degGrid <<EOF\n
               gridtype = lonlat\n
               xsize = 360\n
               ysize = 180\n
               xfirst = -179.5\n
  ...
  clim.mod.files<- list.files(paste("~/", cd.to.path.use, sep = ""), pattern = pattern.use)
  ...
  for(j in seq_along(clim.mods.df$File.Long)){
    ...
    run.command<- paste(paste("cd", paste("~/", "'", cd.to.path.use, "'", sep = ""), sep = " "), 
  ...
  # Save the dataframe
  write_csv(clim.mods.df, path = paste("~/", cd.to.path[[i]], "ClimModelData_ECW.csv", sep = ""))
}
```
...
clim.mods.hist<- read_csv(paste("~/Box/RES Data/CMIP5_SST/Historical/", "ClimModelData.csv", sep = ""))
...
for(i in seq_along(clim.mods.hist.groups)){
  ...
  for(j in seq_along(clim.mods.sub$File.Regrid.Masked.Focal)){
    ...
    rast.temp<- raster::stack(paste("~/Box/RES Data/CMIP5_SST/Historical/", 
  ...
  writeRaster(rast.stack.out, filename = paste("~/Box/RES Data/CMIP5_SST/Historical/", unique(clim.mods.hist$Group)[i], ".grd", sep = ""), overwrite = TRUE)
  
  print(paste(unique(clim.mods.hist$Group)[i], " is done!", sep = ""))
}

## Alright, now the CMP45 and CMP85 projections
clim.mods.rcp.paths<- list("RCP85" = "~/Box/RES Data/CMIP5_SST/RCP85/", "RCP45" = "~/Box/RES Data/CMIP5_SST/RCP45/")
clim.hist.rasts<- list.files(path = "~/Box/RES Data/CMIP5_SST/Historical", pattern = "*.*grd", full.names = TRUE)
...
source(paste(lab.func.path, "ClimateSSTFunctions.R", sep = ""))
...
saveRDS(clim.rast.ts, "~/Box/RES Data/CMIP5_SST/ProcessedSSTProjectionsWithRasters_ECW.rds")
...
saveRDS(clim.out, "~/Box/RES Data/CMIP5_SST/ProcessedSSTProjections_ECW.rds")
```

#### Depth
```{r}
depth<- raster(paste(res.data.path, "Shapefiles/NEShelf_Etopo1_bathy.tiff", sep = ""))
```
...
dailymu.stack<- raster::stack(paste(proj.path, "Data/OISSTThroughFeb2020.grd", sep = ""))
...
temp.pre1982<- trawl.pred.vals.sp %>%
  dplyr::filter(., EST_DATE < "1982-01-01") %>%
  st_drop_geometry()
temp.pre1982$SEASONALMU.OISST<- rep(NA, nrow(temp.pre1982))
...
saveRDS(trawl.dat.full, file = paste(proj.path, "Data/ECWmodel.dat.rds", sep = ""))
```

## Model fitting, evaluation and validation
```{r}
# Getting data together into a training and testing split
dat<- readRDS(paste(proj.path, "Data/ECWmodel.dat.rds", sep = "")) %>%
...
fall.rescale.df<- data.frame(SEASON = "FALL", mean.t = base.temp.mean.f, sd.t = base.temp.sd.f, mean.depth = base.depth.mean.f, sd.depth = base.depth.sd.f)
...
spring.rescale.df<- data.frame(SEASON = "SPRING", mean.t = base.temp.mean.sp, sd.t = base.temp.sd.sp, mean.depth = base.depth.mean.sp, sd.depth = base.depth.sd.sp)
...
spring.preds<- readRDS(paste(proj.path, "Data/spring.rast.preds.rds", sep = ""))
names(spring.preds)[28]<- "DEPTH"
fall.preds<- readRDS(paste(proj.path, "Data/fall.rast.preds.rds", sep = ""))
names(fall.preds)[28]<- "DEPTH"
...
for(i in 1:nrow(dat.full)){
  ...
  # Else keep going
  saveRDS(dat.use$mod.fitted.p[[1]], file = paste(proj.path, "Results/gamfitpres", tolower(dat.use$COMNAME), "_", tolower(dat.use$SEASON), ".rds", sep = ""))
  saveRDS(dat.use$mod.fitted.b[[1]], file = paste(proj.path, "Results/gamfitbio", tolower(dat.use$COMNAME), "_", tolower(dat.use$SEASON), ".rds", sep = ""))
...
write.csv(mod.results, file = paste(proj.path, "Results/mod.results.csv", sep = ""))

```     

## Model predictions
```{r}
res.files.pres<- list.files(paste(proj.path, "Results", sep = ""), "gamfitpres", full.names = TRUE)
res.files.bio<- list.files(paste(proj.path, "Results", sep = ""), "gamfitbio", full.names = TRUE)

mod.results<- read.csv(paste(proj.path, "Results/mod.results.csv", sep = ""))
...
saveRDS(result, file = paste(proj.path, "Results/SDMPredictions.rds", sep = ""))
```
...
clim.out<- readRDS("~/Box/RES Data/CMIP5_SST/ProcessedSSTProjections_ECW.rds")
...
if(FALSE){
  write_csv(temp, paste(proj.path, "Results/CMIP5Projections.csv", sep = ""))
}
...
# Create some masks...
gom<- st_read(paste(res.data.path, "Shapefiles/GulfOfMainePhysioRegions/PhysioRegions_WGS84.shp", sep = ""))
bof<- gom %>%
...
for(i in seq_along(regions)){
  ...
  rcp85plot$Year.Model<- as.integer(rcp85plot$Year) - 1982
   ...
   rcp85.adj.r2.full<- paste("Adj R2 = ", round(summary(rcp85.trend)$adj.r.squared, 3), sep = "")

   # Fitted model formula
   rcp85.my.formula.full<- paste("RCP85 Mean SST = ", signif(round(rcp85.trend$coef[[1]], 3), 5), " + ", signif(round(rcp85.trend$coef[[2]], 3), 5), "*Year", sep = "")
  ...
  rcp45.trend<- lm(SST.YrMean ~ Year.Model, data = rcp45plot)
   ...
   rcp45.adj.r2.full<- paste("Adj R2 = ", round(summary(rcp45.trend)$adj.r.squared, 3), sep = "")

   # Fitted model formula
   rcp45.my.formula.full<- paste("RCP45 Mean SST = ", signif(round(rcp45.trend$coef[[1]], 3), 5), " + ", signif(round(rcp45.trend$coef[[2]], 3), 5), "*Year", sep = "")
...
ggsave(paste(proj.path, "Results/CMIPSSTProjections.jpg", sep = ""), width = 11, height = 8)

# Save the csv
write_csv(rcp.data, paste(proj.path, "Results/CMIPSSTProjections.csv", sep = ""))
```
...
bstrat<- st_read(paste(res.data.path, "Shapefiles/BottomTrawlStrata/BTS_Strata.shp", sep = "")) 
...
ggsave(paste(proj.path, "Results/StudyArea.jpg", sep = ""), plot.out, width = 8, height = 11, dpi = 400)

# Results — Filtering, always run -----------------------------------------------
# Real data
mod.res<- read_csv(paste(proj.path, "Results/mod.results.csv", sep = "")) %>%
...
func.groups<- read.csv(paste(proj.path, "Data/JHareSppFunctionalGroup.csv", sep = ""))
...
ggplot2::ggsave(filename = paste(proj.path, "Results/FallTaylorDiagram", ".jpg", sep = ""), plot = out.f, width = 18, height = 10, units = "in")
...
ggplot2::ggsave(filename = paste(proj.path, "Results/SpringTaylorDiagram", ".jpg", sep = ""), plot = out.s, width = 18, height = 10, units = "in")

# Results — SDM shelfwide and regional changes ----------------------------
# Read in projections
results<- read_rds(paste(proj.path, "Results/SDMPredictions.rds", sep = "")) # This should have everything we need. 
...
if(FALSE){
  ...
  write_csv(preds.out, paste(proj.path, "Results/SDMPredictions.csv", sep = ""))
}
...
nelme<- as(st_read(paste(res.data.path, "Shapefiles/NELME_regions/NELME_sf.shp", sep = "")), "Spatial")
...
res$Change[is.nan(res$Change)]<- NA
...
for(j in seq_along(levels(df$SEASON))){
  ...
  ggplot2::ggsave(filename = paste(proj.path, "Results/", names(res.plot.all)[2], levels(df$SEASON)[j], ".jpg", sep = ""), plot = plot.out, width = 11, height = 8, units = "in")
}
...
for(i in seq_along(seasons)){
  ...
  ggplot2::ggsave(filename = paste(proj.path, "Results/", seasons[i], ".jpg", sep = ""), plot = plot.means, width = 11, height = 8, units = "in")
}
...
land<- st_read(paste(res.data.path, "Shapefiles/ne_50m_land/ne_50m_land.shp", sep = "")) 
...
out.path.use<- paste(proj.path, "Results/", sep = "")
...
for(i in seq_along(spp)){
  ...
  ggsave(paste(proj.path, "Results/", tolower(spp.use), "_ProjectedPercentBioChanges.tiff", sep = ""), plot.out, dpi = 400)
}
```